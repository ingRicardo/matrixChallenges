...
void recursion(int &direct ,int **grid, bool **visited, int h, int w, int row, int col,bool R,bool D,bool L, bool U, int &max)
{
        //R,D,L,U
        //0,1,2,3
        ...
        if(isValid(grid,visited,h,w,nextRow,nextCol)  )
        {
            ...
            if(direct != deltaID){
                ...
                direct = deltaID;
                    ...
                    if( deltaID == 0 && R || !R && deltaID == 1 || !D && deltaID ==2 || !L && deltaID == 3 
                    || R && D && L && U && deltaID == 1 )
                    {
                        recursion(direct,grid,visited,h,w,nextRow,nextCol,R,D,L,U,max);
                    }
 ...
 bool stop =false;
    for(int row =0 ; row < h; row++)
    {
        for(int col = 0; col< w; col++)
        {
            ...
            {
                recursion(direct,grid,visited,h,w,row, col,R,D,L,U,max);
                stop =true;
